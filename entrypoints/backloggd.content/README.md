...
4. **Data Scraping** — Fetches every page of the user's games collection from Backloggd in parallel, by making HTTP requests and parsing the returned HTML with `DOMParser`. Backloggd 1.18's paginated and infinite-scroll modes use different page sizes (40 and 70 respectively); the first response's rendered-card count and total-games label are used to resolve every direct `?page=` request without relying on scroll events.
