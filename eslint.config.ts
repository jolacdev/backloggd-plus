...
import {
  booleanPrefixes,
  importNoRestrictedPathsZoneRestrictions,
  importRulesOff,
...
export default [
  ...
  // SETTINGS: Detect React version automatically for `eslint-plugin-react` rules.
  {
    settings: {
      'import/resolver': {
        // NOTE: Needed by `eslint-plugin-import` with `eslint-plugin-resolver-typescript` to resolve TypeScript paths and aliases.
        typescript: {},
      },
      react: {
        version: 'detect',
...

  {
    ...
    rules: {
      ...
      // Enforces architectural boundaries by disallowing imports between restricted paths.
      'import/no-restricted-paths': [
        2,
        {
          zones: importNoRestrictedPathsZoneRestrictions,
        },
      ],
      ...
      'perfectionist/sort-imports': [
        2,
        {
          ...
          // Map custom path aliases to the 'internal' group to ensure correct import sorting.
          // NOTE: This should match Vite and TypeScript path aliases.
          internalPattern: [
            '@background/.+',
            '@content/.+',
            '@globalShared/.+',
